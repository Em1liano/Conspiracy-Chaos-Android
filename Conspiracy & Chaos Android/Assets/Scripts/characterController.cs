    ...
    private void Awake()
   {
    ...
    private void OnLevelWasLoaded(int level)
    {
        ...
        if (SceneManager.GetActiveScene().name == "Ice")
        {
            canvas.SetActive(true);
            pointLight.SetActive(false);
            transform.position = new Vector2(12.05f, -0.6f);
        }
        ...
        if (SceneManager.GetActiveScene().name == "Boss Volcano")
        {
            canvas.SetActive(true);
            pointLight.SetActive(false);
            transform.position = new Vector2(-14.51f, 4.0f);
        }
    }
    ...
    void Update()
    {
        if (grounded && CrossPlatformInputManager.GetButtonDown("Jump")) //czy wcisniety button
        {
            grounded = false;
        ...
        if (CrossPlatformInputManager.GetButtonDown("Fire2") && bullet)
        {
            //SpriteRenderer spriteRenderer = gameObject.GetComponent<SpriteRenderer>();
    ...
    void FixedUpdate() //fizyka zawsze przez FIXED
    {
        ...
        {
            RB.gravityScale = 1f;
            climbVelocity = climbSpeed * CrossPlatformInputManager.GetAxisRaw("Vertical");
        ...
        float move = CrossPlatformInputManager.GetAxisRaw("Horizontal"); // edit->project settings->input
                                                                         //czyli WSAD;
        Debug.Log(move);                                                 //float move2 = Input.GetAxis("Vertical");
        ...
        if (CrossPlatformInputManager.GetButtonDown("Fire2") && fireRate > 0 && isUsingBow == true && grounded == true)
        {
        ...
        if (CrossPlatformInputManager.GetButtonDown("Fire1") && isUsingBow == false && grounded == true)
        {
            Attack();
        }
